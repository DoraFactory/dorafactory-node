...
# parachain
nohup ./dorafactory-polkadot -d ./data/dorafactory --alice --force-authoring --collator --discover-local --rpc-cors=all --ws-port 8844 --rpc-port 9933 --port 40041 --chain ./config/dorafactory-dev.json -llog=info -lruntime=debug,xcm=trace,jsonrpsee=trace --  --chain ./config/rococo-local-raw.json --discover-local --port 40042 > data/log.2115 2>&1 &
