...
newSudo='
{
  "sudo":"5DFuEcgNPjBmrJncevFzHmrNn52G5JC5aoTJPstnbRsyBRu2"
}'
...
echo "build dorafactory polkadot chainspec"

./dorafactory-polkadot build-spec --disable-default-bootnode --chain dev >  ./config/dorafactory-dev.json
./dorafactory-polkadot export-genesis-state --chain ./config/dorafactory-dev.json > ./config/dorafactory-dev.genesis
./dorafactory-polkadot export-genesis-wasm --chain ./config/dorafactory-dev.json > ./config/dorafactory-dev.wasm
...
jq 'setpath(["name"]; "DoraFactory Rococo Testnet")' |
